#!/usr/bin/env node
// relock-app.mjs — re-seal apps/<id>/holospace.lock.json after editing an app. Mirrors build-app.mjs
// computeApp EXACTLY (same links, root fields, order) so only genuinely-changed files move; resolves
// _shared from the OS2 runtime (os/usr/lib/holo). Generalizes relock-forge.mjs to any app.
//
