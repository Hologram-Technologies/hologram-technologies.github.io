...
// ── share-link carriage: an attestation rides a shared link as att=<base64url> (the same grain holo-proof
//    uses), so "open this κ" can carry "and here is the proof of what it is". The relying party decodes and
//    verifies BEFORE mounting (fail-closed). base64URL (no +,/,= ) so it survives a URL QUERY param too —
//    URLSearchParams turns a '+' into a space, which would corrupt plain base64. No server. ──
function _b64uEnc(str) { const b = (typeof btoa !== "undefined") ? btoa(unescape(encodeURIComponent(str))) : Buffer.from(str, "utf8").toString("base64"); return b.replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, ""); }
function _b64uDec(s) { let t = String(s).replace(/-/g, "+").replace(/_/g, "/"); t += "===".slice((t.length + 3) % 4); return (typeof atob !== "undefined") ? decodeURIComponent(escape(atob(t))) : Buffer.from(t, "base64").toString("utf8"); }
export function encodeAttestationLink(record) { try { return "#att=" + _b64uEnc(JSON.stringify(record)); } catch { return ""; } }
export function decodeAttestationLink(hashOrValue) {
  try {
    const s = String(hashOrValue || "");
    const m = s.match(/[#&?]att=([^&#]+)/);            // a #att= / &att= / ?att= fragment …
    const enc = m ? m[1] : s;                          // … or a bare value (already URL-decoded by the caller)
    return JSON.parse(_b64uDec(enc));
  } catch { return null; }
}

// shareSigner() — a NON-PROMPTING signer for AUTO-attestation on share. Uses the operator if the shell
// already exposes an unlocked signer (window.HoloIdentity.signer), else mints an ephemeral did:key. Either
// yields a self-verifying, tamper-evident Evidence: for share-to-run the value is the bound subject +
// fail-closed verify-before-mount, so we never block a share to prompt for a key. (Operator PROVENANCE on
// a share — when it matters — is a deliberate step-up act, not this silent path.)
async function shareSigner() {
  try { if (typeof window !== "undefined" && window.HoloIdentity && typeof window.HoloIdentity.signer === "function") { const s = await window.HoloIdentity.signer(); if (s && s.kappa && typeof s.sign === "function") return s; } } catch {}
...
// attestShareLink(link, subject, { closure }) — append att=<base64url> to a share link so it self-verifies
// on arrival. Placed in the QUERY (before any #hash), so a #k=/#wks= resume fragment is left untouched.
// Non-prompting + FAIL-SOFT: any error returns the link unchanged — sharing must never break (Law: simplicity).
export async function attestShareLink(link, subject, { closure = null } = {}) {
  try {
    if (!link || !subject) return link;
    const rec = await attest({ subject, closure }, await shareSigner());
    const enc = _b64uEnc(JSON.stringify(rec));
    const u = new URL(link, (typeof location !== "undefined" && location.href) || "https://holo.local/");
    u.searchParams.set("att", enc);                    // query param survives the hash untouched + base64url-safe
    return u.toString();
  } catch { return link; }
}
...
if (typeof window !== "undefined" && !window.HoloAttest) {
  window.HoloAttest = Object.freeze({ attest, verify: verifyAttestation, build: buildAttestation, attachTee, challengeFor: challengeForAttestation, encodeLink: encodeAttestationLink, decodeLink: decodeAttestationLink, shareLink: attestShareLink });
}
...
export default { attest, buildAttestation, attachTee, verifyAttestation, challengeForAttestation, encodeAttestationLink, decodeAttestationLink, attestShareLink };
