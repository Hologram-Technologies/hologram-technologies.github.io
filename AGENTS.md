...
| Provable ownership & settlement — **built, witnessed** | `holo-own.mjs` · `tools/holo-own-witness.mjs` · MCP `own_verify` / `own_settle` / `own_passport` — re-derive a Title chain to prove who owns an object now, and settle value against a *proven* head (pay-for-proven; a forged or tampered title releases nothing) | 0053 |
